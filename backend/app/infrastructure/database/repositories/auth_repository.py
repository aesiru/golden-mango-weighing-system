...
class AuthRepository:
    ...
    async def get_entity_permissions(
        self, role_ids: List[str], entity_name: Optional[str] = None
    ) -> List[EntityPermission]:
        """Get permissions for multiple roles, optionally filtered by entity."""
        query = select(EntityPermission).where(EntityPermission.role_id.in_(role_ids))
        if entity_name:
            query = query.where(EntityPermission.entity_name == entity_name)
