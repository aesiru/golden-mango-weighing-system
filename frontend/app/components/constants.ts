...
export const customNodesGraph = css`
  ...
  --unovis-graph-node-identity: color-mix(
    in srgb,
    var(--vis-color0) 90%,
    transparent
  );
  --unovis-graph-node-network: color-mix(
    in srgb,
    var(--vis-color1) 90%,
    transparent
  );
  --unovis-graph-node-resource: color-mix(
    in srgb,
    var(--vis-color2) 90%,
    transparent
  );
  --unovis-graph-node-compute: color-mix(
    in srgb,
    var(--vis-color3) 90%,
    transparent
  );
  --unovis-graph-node-secret: color-mix(
    in srgb,
    var(--vis-color4) 90%,
    transparent
  );
  --unovis-graph-node-finding: color-mix(
    in srgb,
    var(--vis-color5) 90%,
    transparent
  );
  ...
  --vis-graph-link-stroke-opacity: 1;
...
`;
export const nodeHighlightBackground = css`
  label: node-highlight-background;
`;

// Node Aggregation
export const nodeAggregationBackground = css`
  label: node-aggregation-background;
`;
export const nodeAggregationText = css`
  label: node-aggregation-text;
`;

// Watchlist
export const nodeWatchlistBackground = css`
  label: node-watchlist-background;
`;
export const nodeWatchlistIcon = css`
  label: node-watchlist-icon;
`;

// Session Count
export const nodeSessionCountBackground = css`
  label: node-session-count-background;
`;
export const nodeSessionCountText = css`
  label: node-session-count-text;
`;

// Findings
export const nodeFinding = css`
  label: node-finding;
`;
export const nodeFindingBackground = css`
  label: node-finding-background;
`;
export const nodeFindingText = css`
  label: node-finding-text;
`;

// Enrichment
export const nodeEnrichments = css`
  label: node-enrichments;
`;
export const nodeEnrichment = css`
  label: node-enrichment;
`;
...
`;
export const nodeEnrichmentIcon = css`
  label: node-enrichment-icon;
`;
...
// Swimlanes
export const swimlaneRect = css`
  label: swimlane-rect;
`;
export const swimlaneLabel = css`
  label: swimlane-label;
`;
export const swimlaneLabelBackground = css`
  label: swimlane-label-background;
`;
...
export const updateSwimlanes = (
  ...
  g.selectAll<SVGGElement, CustomGraphSwimlane>(`.${swimlaneRect}`).attr(
    "transform",
    `translate(${transform.translate.x}, 0) scale(${transform.scale.x}, 1)`,
  );

  g.selectAll<SVGGElement, CustomGraphSwimlane>(`.${swimlaneLabel}`).attr(
    "transform",
    `translate(${transform.translate.x}, 0) scale(${transform.scale.x}, 1)`,
  );

  g.selectAll<SVGTextElement, CustomGraphSwimlane>(
    `.${swimlaneLabelText}`,
  ).attr(
    "transform",
    (lane: CustomGraphSwimlane) =>
      `scale(${1 / transform.scale.x}, 1) translate(${lane.xCenter * transform.scale.x}, 0)`,
  );

  g.selectAll<SVGRectElement, CustomGraphSwimlane>(
    `.${swimlaneLabelBackground}`,
  ).attr(
    "transform",
    (lane: CustomGraphSwimlane) =>
      `scale(${1 / transform.scale.x}, 1) translate(${lane.xCenter * transform.scale.x}, 0)`,
...
export const renderSwimlanes = (
  ...
  const nodesGrouped = group(
    nodes,
    (d: GraphNode<CustomGraphNode, CustomGraphLink>) => d.group ?? d.type,
  );
  const largeSvgNumber = 9999;
  const swimlanes = Array.from(nodesGrouped.entries()).map(
    (
      [key, nodes]: [
        string | undefined,
        GraphNode<CustomGraphNode, CustomGraphLink>[],
      ],
      i: number,
    ) => ({
      index: i,
      ...
      xCenter: mean(
        nodes,
        (d: GraphNode<CustomGraphNode, CustomGraphLink>) => d.x,
      ) as number,
      xMin: min(
        nodes,
        (d: GraphNode<CustomGraphNode, CustomGraphLink>) => d.x,
  ...
  for (const [index, lane] of swimlanes.entries()) {
    ...
    switch (i) {
      ...
      case 0:
        lane.x1 = -largeSvgNumber;
        const nextLane =
          i + 1 < swimlanes.length ? swimlanes[i + 1] : undefined;
        lane.x2 = nextLane ? (lane.xMax + nextLane.xMin) / 2 : largeSvgNumber;
        break;
      case swimlanes.length - 1:
        const prevLane = i - 1 >= 0 ? swimlanes[i - 1] : undefined;
        lane.x1 = prevLane ? (lane.xMin + prevLane.xMax) / 2 : -largeSvgNumber;
        lane.x2 = largeSvgNumber;
        break;
      default:
        const prevLaneDefault = i - 1 >= 0 ? swimlanes[i - 1] : undefined;
        const nextLaneDefault =
          i + 1 < swimlanes.length ? swimlanes[i + 1] : undefined;
        lane.x1 = prevLaneDefault
          ? (lane.xMin + prevLaneDefault.xMax) / 2
          : lane.xMin;
        lane.x2 = nextLaneDefault
          ? (lane.xMax + nextLaneDefault.xMin) / 2
          : lane.xMax;
  ...
  const svgSwimlaneRects = g
    .selectAll<SVGRectElement, CustomGraphSwimlane>(`.${swimlaneRect}`)
    .data(swimlanes);

  const svgSwimlanesRectsEnter = svgSwimlaneRects
    .enter()
  ...
  (svgSwimlanesRectsEnter as any).transition().style("opacity", 1);

  const svgSwimlanesRectsMerged = svgSwimlaneRects.merge(
    svgSwimlanesRectsEnter,
  );
  svgSwimlanesRectsMerged
    .attr("x", (lane: CustomGraphSwimlane) => lane.x1)
    .attr("width", (lane: CustomGraphSwimlane) => lane.x2 - lane.x1)
    .attr("y", -largeSvgNumber)
    .attr("height", largeSvgNumber * 2)
    .style("fill", (lane: CustomGraphSwimlane) =>
      lane.index % 2
        ? "var(--unovis-background-primary)"
        : "var(--unovis-background-secondary)",
    );

  // D3 types are not correct
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (svgSwimlaneRects.exit() as any).transition().style("opacity", 0).remove();

  // Labels in the foreground
  const svgSwimlaneLabels = g
    .selectAll<SVGGElement, CustomGraphSwimlane>(`.${swimlaneLabel}`)
    .data(swimlanes);

  const svgSwimlaneLabelsEnter = svgSwimlaneLabels
    .enter()
    .append("g")
  ...
  // D3 types are not correct
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (svgSwimlaneLabelsEnter as any).transition().style("opacity", 1);

  svgSwimlaneLabelsEnter.append("rect").attr("class", swimlaneRect);

  svgSwimlaneLabelsEnter.append("rect").attr("class", swimlaneLabelBackground);

  svgSwimlaneLabelsEnter.append("text").attr("class", swimlaneLabelText);

  // D3 types are not correct
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (svgSwimlaneLabels.exit() as any).transition().style("opacity", 0).remove();

  const svgSwimlaneLabelsMerged = svgSwimlaneLabels.merge(
    svgSwimlaneLabelsEnter,
  );
  svgSwimlaneLabelsMerged
    .selectAll<SVGRectElement, CustomGraphSwimlane>(
      `.${swimlaneLabelBackground}`,
    )
    .data((lane: CustomGraphSwimlane) => [lane])
    .attr("y", "94.1%")
    .attr("height", "4%")
    .attr("rx", 8)
    .attr("ry", 8)
  ...
  svgSwimlaneLabelsMerged
    .selectAll<SVGTextElement, CustomGraphSwimlane>(`.${swimlaneLabelText}`)
    .data((lane: CustomGraphSwimlane) => [lane])
    .text((lane: CustomGraphSwimlane) => lane.label)
    .attr("y", "97%");

  // Set the background size
  const labelPadding = 5;
  svgSwimlaneLabelsMerged.each(
    (_: any, i: number, nodes: SVGGElement[] | ArrayLike<SVGGElement>) => {
      const node = nodes[i];
      if (!node) return;
...
>(
  ...
  // Node Aggregation
  g.append("rect")
    .attr("class", nodeAggregationBackground)
    .classed(nodeCircleLabelBackground, true);
  g.append("text")
    .attr("class", nodeAggregationText)
    .classed(nodeCircleLabelText, true);

  // Node Session Count
  g.append("rect")
    .attr("class", nodeSessionCountBackground)
    .classed(nodeCircleLabelBackground, true);
  g.append("text")
    .attr("class", nodeSessionCountText)
    .classed(nodeCircleLabelText, true);

  // Node Watchlist
  g.append("circle")
    .attr("class", nodeWatchlistBackground)
    .classed(nodeCircleLabelBackground, true);
...
>(
  ...
  const nodeIconSize =
    getNumber(d, config.nodeIconSize, d._index) ?? 2.5 * Math.sqrt(nodeSize);
  ...
  g.select<SVGUseElement>(`.${nodeIcon}`)
    .attr(
      "href",
      getString(d, config.nodeIcon as StringAccessor<N>, d._index) as string,
    )
  ...
  const aggregationPos = [
    nodeCircleLabelPlacementDistance * Math.cos(Math.PI / 4),
    -nodeCircleLabelPlacementDistance * Math.sin(Math.PI / 4),
  ];
  const aggregationCountVisibility = (
    d.aggregationCount ? null : "hidden"
  ) as string;
  g.select<SVGTextElement>(`.${nodeAggregationText}`)
    .attr("x", aggregationPos[0] ?? 0)
    .attr("y", aggregationPos[1] ?? 0)
  ...
  g.select<SVGRectElement>(`.${nodeAggregationBackground}`)
    ...
    .attr(
      "transform",
      `translate(${-DEFAULT_CIRCLE_LABEL_SIZE}, ${-DEFAULT_CIRCLE_LABEL_SIZE})`,
    )
    .attr("visibility", aggregationCountVisibility);
  ...
  const sessionCountBackgroundWidth = Math.max(
    numSessionsText.length * 7.5,
    DEFAULT_CIRCLE_LABEL_SIZE * 2,
  );
  g.select<SVGTextElement>(`.${nodeSessionCountText}`)
    .attr(
      "x",
      (sessionCountPos[0] ?? 0) +
        sessionCountBackgroundWidth / 2 -
        DEFAULT_CIRCLE_LABEL_SIZE,
    )
    .attr("y", sessionCountPos[1] ?? 0)
    .attr("text-anchor", "middle")
    .attr("dominant-baseline", "central")
    .text(d.numSessions ?? "")
  ...
  g.select<SVGUseElement>(`.${nodeWatchlistIcon}`)
    ...
    .attr("x", (watchlistPos[0] ?? 0) - DEFAULT_CIRCLE_LABEL_SIZE / 2)
    .attr("y", (watchlistPos[1] ?? 0) - DEFAULT_CIRCLE_LABEL_SIZE / 2)
  ...
  g.select<SVGCircleElement>(`.${nodeWatchlistBackground}`)
    ...
    .attr("cx", watchlistPos[0] ?? 0)
    .attr("cy", watchlistPos[1] ?? 0)
    .attr("visibility", watchlistVisibility);

  const alertsData = Object.entries(d.numFindings ?? {}).filter(
    ([_, v]) => v > 0,
  );
  const alerts = g
    .selectAll<SVGGElement, [string, number]>(`.${nodeFinding}`)
    .data(alertsData);
  ...
  alertsEnter
    ...
    .style("fill", ([severity]) => {
      switch (severity) {
        case "low":
          return "var(--unovis-severity-low)";
        case "medium":
          return "var(--unovis-severity-medium)";
        case "high":
          return "var(--unovis-severity-high)";
        case "critical":
          return "var(--unovis-severity-critical)";
        default:
          return "black";
      }
    });
  ...
  alertsEnter
    ...
    .attr("transform", (_, i) => {
      ...
      const angle =
        -Math.PI / 1.33 -
        index * 2.66 * Math.atan2(DEFAULT_CIRCLE_LABEL_SIZE, r);
  ...
  const enrichments = g
    .selectAll<SVGGElement, CustomGraphNodeStatus>(`.${nodeEnrichment}`)
    .data(enrichmentsData);

  const enrichmentsEnter = enrichments
    .enter()
    .append("g")
  ...
  enrichmentsEnter.append("use").attr("class", nodeEnrichmentIcon);

  const enrichmentsUpdate = enrichmentsEnter.merge(enrichments);
  enrichmentsUpdate.attr("transform", (_, i) => {
    const r = nodeSize * 1.75;
    const dx =
      -(enrichmentsData.length - 1) * DEFAULT_CIRCLE_LABEL_SIZE +
      i * 2 * DEFAULT_CIRCLE_LABEL_SIZE;
  ...
  const nodeLabelElement = g
    .select<SVGTextElement>(`.${nodeLabel}`)
    .attr("y", labelPlacementDistance)
    .text(trimString(d.label));

  const nodeSubLabelElement = g
    .select<SVGTextElement>(`.${nodeSubLabel}`)
    .attr("y", labelPlacementDistance)
    .attr("dy", "1.25em")
